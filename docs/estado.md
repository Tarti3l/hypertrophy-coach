...
| [21] Eliminar un entrenamiento finalizado | en revisión | #33 | Sale del hallazgo del [13]: era el único dato de la app que no se podía borrar. Sección "Tus sesiones" en Progreso con las 20 últimas sesiones terminadas (fecha, series, ejercicios, duración) y "Eliminar" en **dos pasos** ("Eliminar" → "Sí, eliminar" / "Cancelar"), el mismo patrón que ya usa `SyncErrorsScreen` para descartar — `Alert.alert` no funciona en la versión web. **Sin migración**: la política de `workouts` ya era `for all` y `workout_sets.workout_id` ya tenía `on delete cascade`, así que las series se van con la sesión y nada queda huérfano. Al borrar se recarga también el gráfico, que sale de esas mismas series. `pnpm typecheck` verde. Verificado contra la base real sin tocar datos: el `select` anidado devuelve 200 (si el embed estuviera mal daría 400 y la lista mostraría "0 series" en silencio), y un `DELETE` anónimo borra 0 filas, o sea que RLS impide borrar sesiones ajenas. **Falta la verificación del dueño en vivo**: borrar una sesión real y confirmar que desaparece al recargar |
