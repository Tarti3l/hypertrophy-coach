...
| [18] Seguimiento de peso corporal | en revisión | #29 | Migración 00024 (`body_weight_logs`, una fila por día con `unique (user_id, measured_on)`), pantalla «Mi peso» desde Alimentación con fecha de hoy preseleccionada, chips Hoy/Ayer, corrección de fecha y valor, y evolución con el mismo `LineChart` que ya usa Progreso. No toca `user_profiles` ni el cálculo de macros. El peso del onboarding se muestra aparte, etiquetado como declarado, y no entra en la evolución. `pnpm typecheck` verde; `pnpm db:status` muestra 00024 solo en local. **Falta que el dueño corra `pnpm db:push` y verifique en vivo**: sin la tabla aplicada no se pudo probar el recorrido real (guardar, corregir, recargar) |
