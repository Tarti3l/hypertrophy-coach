...
| [4] Sustituir los consumos ficticios por los registros reales | en revisión | #8 | Implementado en `item-4-Codex`: consumos desde `meal_entries`, con estados de carga, vacío confirmado e indisponibilidad. Falta verificar en dispositivo/servidor: Codex no llegó a levantar el server en su worktree, así que el criterio de aceptación no se comprobó de forma visual — pendiente registrar una comida, ver que los totales reflejen lo cargado, y que recargar los conserve |
