...
--   1. "Anterior: 20 kg × 12" en el SetTracker, que es la referencia con la que el
--      usuario decide cuánto cargar hoy.
--   2. El gráfico de fuerza (aunque ahí toma el máximo, así que aguanta).
--   3. La sugerencia de progresión, que promedia series.
--
-- Marcarlas permite excluirlas de esas tres lecturas sin perder el dato: saber con qué
-- calentaste la última vez es útil justo al empezar el ejercicio.
--
...
comment on column public.workout_sets.is_warmup is
  'true = serie de aproximación, no cuenta como volumen efectivo ni como referencia de progresión.';

-- Las consultas de referencia e historial filtran por ejercicio y piden solo las
-- efectivas. Un índice parcial las sirve sin arrastrar los calentamientos.
...
-- cliente viejo (o un entrenamiento encolado antes de esta migración) manda el payload
