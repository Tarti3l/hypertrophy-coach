...
| [21] Eliminar un entrenamiento finalizado | hecho | #33 | Sale del hallazgo del [13]: era el único dato de la app que no se podía borrar. Sección "Tus sesiones" en Progreso con las 20 últimas sesiones terminadas (fecha, series, ejercicios, duración) y "Eliminar" en **dos pasos** ("Eliminar" → "Sí, eliminar" / "Cancelar"), el mismo patrón que ya usa `SyncErrorsScreen` para descartar — `Alert.alert` no funciona en la versión web. **Sin migración**: la política de `workouts` ya era `for all` y `workout_sets.workout_id` ya tenía `on delete cascade`, así que las series se van con la sesión y nada queda huérfano. Al borrar se recarga también el gráfico, que sale de esas mismas series. `pnpm typecheck` verde. Verificado contra la base real sin tocar datos: el `select` anidado devuelve 200 (si el embed estuviera mal daría 400 y la lista mostraría "0 series" en silencio), y un `DELETE` anónimo borra 0 filas, o sea que RLS impide borrar sesiones ajenas. Verificación del dueño en vivo: se completó de verdad "Press de pecho en máquina" (4 series, 20 kg x 12) y se finalizó el entrenamiento — quedó 1 sesión en "Esta semana" y racha en 1 día. En "Progreso → Tus sesiones" apareció la fila con fecha/series/duración; "Eliminar" pidió confirmación explícita ("Sí, eliminar" / "Cancelar", no un solo toque) antes de borrar. Tras confirmar, la sesión desapareció de la lista, y tras recargar de verdad (no solo navegar) "Esta semana" volvió a 0 sesiones y la racha a 0 — confirma que el borrado es real en el servidor, no solo en el estado local. Sin errores nuevos en consola. De paso quedó verificado el hallazgo pendiente del [13]: con exactamente una sesión previa de un ejercicio, la pantalla mostró "20 kg × 12" y la sugerencia "Una repetición más" ("La última vez hiciste 12 con 20 kg. Prueba una repetición más antes de subir peso."), y "Anterior: 20 kg × 12" en el registro de series — correcto. No quedó ninguna sesión de prueba en la cuenta real: se borró con esta misma función. |
